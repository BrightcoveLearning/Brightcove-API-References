 ...
 * @apiParam (Request Body Fields) {Number} renditions.audio_bitrate audio bitrate in kbps **Legacy Ingest only**
 ...
 * @apiParam (Request Body Fields) {Number} [renditions.crf] 1-51, not used by default. **Legacy Ingest only**
 * @apiParam (Request Body Fields) {Number} [renditions.decoder_bitrate_cap] In kbps, the max bitrate fed to the decoder **Legacy Ingest only**
 * @apiParam (Request Body Fields) {String} [renditions.encryption_method] encryption_method to use, e.g. `aes-128` **Legacy Ingest only**
 * @apiParam (Request Body Fields) {Number} [renditions.encryption_key_rotation_period=10] use a different key for each set of segments, rotating to a new key after this many segments **Legacy Ingest only**
 * @apiParam (Request Body Fields) {Number} [renditions.fixed_keyframe_interval] Forces a keyframe every X frames, but still allows additional keyframes &mdash; **NOTE** overrides `keyframe_interval` **Legacy Ingest only**
 ...
 * @apiParam (Request Body Fields) {Number} [renditions.frame_rate="(same as source)"] frame rate in frames per second **Legacy Ingest only**
 * @apiParam (Request Body Fields) {Number} [renditions.h264_bframes=0] number of bframes for h.264 **Legacy Ingest only**
 * @apiParam (Request Body Fields) {Number} [renditions.h264_level="(calculated)"] h.264 profile level **Legacy Ingest only**
 * @apiParam (Request Body Fields) {String="baseline","main","high"} [renditions.h264_profile="baseline"] h.264 profile **Legacy Ingest only**
 * @apiParam (Request Body Fields) {Number} [renditions.h264_reference_frames=3] number of h.264 reference frames to use **Legacy Ingest only**
 * @apiParam (Request Body Fields) {Boolean} [renditions.hls_optimized_ts] Time segments optimized for HLS **Legacy Ingest only**
 * @apiParam (Request Body Fields) {Number} [renditions.keyframe_interval] Maximum number of frames between keyframes (default is 250, and overrides keyframe_rate) **Legacy Ingest only**
 * @apiParam (Request Body Fields) {Number} [renditions.keyframe_rate] Maximum number of keyframes per second **Legacy Ingest only**
 ...
 * @apiParam (Request Body Fields) {Boolean} [renditions.live_stream] Whether this will be for live streaming video **Legacy Ingest only**
 ...
 * @apiParam (Request Body Fields) {Number} [renditions.max_video_bitrate] Maximum video bitrate (h.264 only) **Legacy Ingest only**
 * @apiParam (Request Body Fields) {Number} [renditions.max_frame_rate] Limits the frame rate rather than sets it, use as an alternative to frame rate **Legacy Ingest only**
 ...
 * @apiParam (Request Body Fields) {Number} [renditions.skip.min_audio_bitrate] the minimum audio bitrate that the source must have (kbps) **Legacy Ingest only**
 * @apiParam (Request Body Fields) {Number} [renditions.skip.max_audio_bitrate] the maximum audio bitrate that the source must have (kbps) **Legacy Ingest only**
 * @apiParam (Request Body Fields) {Number} [renditions.skip.min_video_bitrate] the minimum video bitrate that the source must have (kbps) **Legacy Ingest only**
 * @apiParam (Request Body Fields) {Number} [renditions.skip.max_video_bitrate] the maximum video bitrate that the source must have (kbps) **Legacy Ingest only**
 ...
 * @apiParam (Request Body Fields) {Number=1-5} [renditions.speed=3] a target transcoding speed. Slower transcoding allows for more advanced file compression, while faster transcoding is possible by skipping some advanced compression features **Legacy Ingest only**
 ...
 * @apiParam (Request Body Fields) {Number} renditions.video_bitrate target video bitrate in kbps **Legacy Ingest only**
 * @apiParam (Request Body Fields) {String} renditions.video_codec target video codec **Legacy Ingest only**
 * @apiParam (Request Body Fields) {Number} renditions.height target frame height in pixels **Legacy Ingest only**
 * @apiParam (Request Body Fields) {Number} renditions.width target frame width in pixels **Legacy Ingest only**
 ...
 * @apiParam (Request Body Fields) {Mixed} packages.renditions for MPEG-DASH, the renditions will be set to the `reference_id` for a single rendition; for other formats, `renditions` is set equal to an array of rendition `reference_id`'s **Legacy Ingest only**
 *
 *
