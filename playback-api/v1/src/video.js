 ...
 * @apiError (Error 403) ACCOUNT_ID FORBIDDEN - The account id in the policy key does not match the account in the api request
 * @apiError (Error 403) DOMAIN FORBIDDEN - The video is restricted from playing on the current domain
 * @apiError (Error 403) CLIENT_GEO FORBIDDEN - The video is restricted from playing in the current geo region; the message will contain additional information about the specific issue. For more details, see the [Playback API Error Reference](http://docs.brightcove.com/en/video-cloud/playback-api/references/error-reference.html)
 * @apiError (Error 403) CLIENT_IP FORBIDDEN - The video is restricted at the current IP address
 * @apiError (Error 403) POLICY_ERROR FORBIDDEN - Error when evaluating the policy key
 * @apiError (Error 403) VIDEO_NOT_PLAYABLE FORBIDDEN - For a single video request, the video exists, but is not allowed to be played now. That could be any of the three reasons that videos are not playable: not sufficiently ingested, not active, not in scheduled date range.
